...
bool SendWebSocketTextFrame(SOCKET sock, const std::string& text) {
    ...
    std::vector<char> frameHeader;
    frameHeader.push_back((char)0x81); // Text frame
    ...
    std::vector<char> fullBuf;
    fullBuf.reserve(frameHeader.size() + text.size());
    fullBuf.insert(fullBuf.end(), frameHeader.begin(), frameHeader.end());
    fullBuf.insert(fullBuf.end(), text.begin(), text.end());

    int total = (int)fullBuf.size();
    int sent = 0;
    while (sent < total) {
        int n = send(sock, fullBuf.data() + sent, total - sent, 0);
        if (n <= 0) return false;
        sent += n;
    }
    return true;
